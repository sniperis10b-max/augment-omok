...
import { gameReducer, createInitialState, isBlocked, BLACK, WHITE, WILD } from './gameReducer.js';
  ...
  useEffect(() => {
    ...
    const unsub = subscribeRoom(online.code, (room) => {
      ...
      if (online.role === 'host' && room.status === 'active' && !gameStartedRef.current) {
        ...
        pendingLocalRef.current = true;
        dispatch({ type: 'START_GAME', aiPlayer: null, difficulty: 'normal' });
      }
    });
    return unsub;
  }, [online]);
...
function SetupScreen({ dispatch, online, setOnline }) {
  const [step, setStep] = useState('mode'); // 'mode' | 'color' | 'difficulty' | 'online-menu' | 'online-host-color' | 'online-waiting' | 'online-join' | 'online-error'
  ...
  async function handleCreateRoom(hostColor) {
    ...
    try {
      ...
      setOnline({ code, localColor: hostColor, role: 'host' });
    ...
    return (
      <div className="page">
        ...
        <div className="setup-options">
          <button className="setup-card" onClick={() => dispatch({ type: 'START_GAME', aiPlayer: null })}>
          ...
          <button className="setup-card" onClick={() => setStep('color')}>
          ...
          <button className="setup-card" onClick={() => setStep('online-menu')}>
        ...
        <div className="setup-options">
          {Object.entries(DIFFICULTIES).map(([key, cfg]) => (
            <button
              ...
              onClick={() => dispatch({ type: 'START_GAME', aiPlayer, difficulty: key })}
            >
