// ⚠️ 이 파일은 src/gameLogic.js를 그대로 복사한 거예요. 클라이언트 로직(오목 승리 판정,
// 렌주 금수 규칙)을 서버(Vercel Serverless Function)에서도 똑같이 검증하기 위해 필요해요.
// src/gameLogic.js를 고치면 이 파일도 반드시 같이 고쳐주세요 (자동 동기화 안 됨).
//
