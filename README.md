...
## 서버 측 착수 검증 (Cloud Functions) — 진행 중, 아직 클라이언트에 연결 안 됨
...
`functions/` 폴더에 이걸 막기 위한 **1단계** 작업을 넣어뒀어요:

- `functions/gameLogic.js`: `src/gameLogic.js`를 그대로 복사한 파일이에요 (승리 판정,
  렌주 금수 규칙). 이 로직을 서버에서도 똑같이 돌려서 검증하기 위해 필요해요.
  **`src/gameLogic.js`를 고치면 이 파일도 반드시 같이 고쳐주세요** (자동 동기화 안 돼요).
- `functions/index.js`: `placeStone`이라는 Cloud Function이 있어요. 방 코드/좌표를
  받아서 — 내 턴이 맞는지, 빈 칸인지, 막힌 칸은 아닌지, 렌주 금수는 아닌지, 승리인지 —
  서버에서 다시 검증해요.

**진행 상황**
- ✅ `functions/index.js`의 `placeStone` 함수가 완성됐어요.
- ✅ **클라이언트가 이제 이 함수를 호출해요**: `src/serverValidation.js`가 온라인 대전에서
  "카드 대상 선택 중이 아닌 평범한 돌 놓기"를 할 때마다 이 함수를 먼저 불러서 확인받고,
  통과해야만 실제로 착수해요. 함수가 배포되지 않았거나 네트워크 문제가 있으면(예:
  아직 `firebase deploy`를 안 한 상태) **안전하게 통과 처리**돼서 게임이 막히지 않아요.
- ❌ 카드 효과(파괴·변환·봉인 등 60개 이상)와 룰렛/챌린지의 특수 규칙은 아직 서버에서
  검증하지 않아요. 순수 "돌 하나 놓기"만 검증 대상이에요. 카드까지 전부 서버로
  옮기는 건 훨씬 큰 작업이라 다음 단계로 남겨뒀어요.
- ❌ 그래서 지금은 `database.rules.json`에서 `rooms`를 여전히 로그인한 사람이면 직접 쓸
  수 있게 열어뒀어요 — 카드 로직 이식이 끝나기 전에 먼저 막아버리면 카드 효과가 전부
  깨져요. (즉, 지금 이 서버 검증은 "브라우저 콘솔로 아예 대놓고 조작하는 것"까지 막는
  건 아니고, 실수나 어설픈 조작을 걸러내는 첫 번째 안전장치예요.)

**개발 중 실제로 잡은 버그**: 처음엔 서버가 "이 수는 안 돼요"를 예외(에러)로 던지는
방식으로 만들었는데, Cloud Function을 아직 배포하지 않았을 때 발생하는 "함수가 없음"
에러와 실제 거부 사유가 **똑같은 에러 코드**로 찍혀서 구분이 안 됐어요. 그 상태로 클라이언트를
연결했다면 함수를 배포하기 전까지 온라인 대전 전체가 막혀버렸을 거예요. 그래서 서버가
거부 사유를 예외 대신 **정상 응답의 필드 값**으로 돌려주는 방식으로 다시 만들어서,
"서버가 진짜로 거부한 것"과 "함수를 아예 못 부른 것"을 명확히 구분하도록 고쳤어요.
...
```
(Firebase CLI 로그인과 프로젝트 설정이 먼저 되어 있어야 해요: `firebase login`,
`firebase use augment-omok`)
